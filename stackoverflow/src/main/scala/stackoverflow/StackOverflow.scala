...
class StackOverflow extends StackOverflowInterface with Serializable:
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] =
    ...
    val answers = postings.filter(_.postingType == 2).filter(_.parentId.isDefined).map(posting => (posting.parentId.get, posting))
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] =
    ...
    def answerHighScore(as: Array[Answer]): HighScore =
      ...
      while i < as.length do
        ...
        i += 1
      highScore
    grouped.flatMap(group =>  group._2).groupByKey().mapValues(scores => answerHighScore(scores.toArray))
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] =
    ...
    val newMeansMap: scala.collection.Map[(Int, Int), (Int, Int)] = vectors.map(vector => (findClosest(vector, means), vector))
      .groupByKey()
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] =
    def computeMedian(scores: Iterable[Int]): Int = {
      val sortedScores = scores.toSeq.sorted
      val mid = sortedScores.size / 2
      if sortedScores.size % 2 == 0 then
        (sortedScores(mid) + sortedScores(mid - 1)) / 2
      else
        sortedScores(mid)
    }
    ...
    val median = closestGrouped.mapValues { vs =>
      val langId = vs.groupBy(_._1).maxBy(_._2.size)._1
      val langLabel: String = langs(langId / langSpread) // most common language in the cluster
      val langPercent: Double =  vs.count(_._1 == langId).toDouble * 100.0 / vs.size // percent of the questions in the most common language
